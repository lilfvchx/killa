...
func (s *slackService) handleEnvelope(encoded string) (string, error) {
	...
	default:
		return "", fmt.Errorf("unsupported slack action %q", header.Action)
	}
	if err != nil {
		return "", err
	}
...
func (s *slackService) handleCheckin(raw []byte) ([]byte, error) {
	req := checkinMessage{}
...
func (s *slackService) handleGetTasking(callbackUUID string, raw []byte) ([]byte, error) {
	req := taskingMessage{}
	if err := json.Unmarshal(raw, &req); err != nil {
		return nil, err
	}
...
func (s *slackService) handlePostResponse(callbackUUID string, raw []byte) ([]byte, error) {
...
func (s *slackService) handleDownload(task mythicrpc.PTTaskMessageTaskData, callback *mythicrpc.MythicRPCCallbackSearchMessageResult, msg fileDownloadMessage) (map[string]any, error) {
	if msg.FileID == "" {
...
func (s *slackService) handleUpload(msg fileUploadMessage) (map[string]any, error) {
