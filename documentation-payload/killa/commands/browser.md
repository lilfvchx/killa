...
|-----------|------|----------|---------|-------------|
| action | choose_one | No | history | `passwords` â€” saved login credentials (Windows only); `cookies` â€” session cookies (Windows only); `history` â€” browsing history; `autofill` â€” form data; `bookmarks` â€” saved URLs |
| browser | choose_one | No | all | `all`, `chrome`, `edge`, or `chromium` â€” which browser(s) to target |
...
- The browser does not need to be closed â€” databases are copied to avoid lock conflicts
