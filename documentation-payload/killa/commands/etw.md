...
The `blind` action is the preferred evasion method â€” it disables a specific provider within a session without stopping the session itself, making detection harder.
...
| session_name | For stop/blind/query/enable | â€” | Target trace session name |
| provider | For blind/enable | â€” | Provider GUID or shorthand name |

### Actions

- **sessions** â€” List all active ETW trace sessions with security relevance classification
- **providers** â€” Enumerate all registered ETW providers, highlighting security-relevant ones
- **stop** â€” Stop an entire ETW trace session (ControlTrace API). Disables all telemetry from that session.
- **blind** â€” Surgically disable a specific provider within a trace session (EnableTraceEx2 API). The session remains active but the targeted provider no longer generates events. Stealthier than `stop`.
- **query** â€” Get detailed information about a specific trace session: buffer sizes, events lost, log file mode, flush timer, and security relevance.
- **enable** â€” Re-enable a previously blinded ETW provider within a session. Restores event generation at TRACE_LEVEL_VERBOSE with all keywords. Use for cleanup after operations.
...
- **`blind` is preferred over `stop`** â€” it removes a single provider while the session continues to run, making detection harder
- **`stop` is the nuclear option** â€” it kills the entire session, which may be noticed by monitoring
- Requires Administrator or SYSTEM privileges for stop/blind actions
- Common high-value targets for blinding:
  - **Sysmon** â€” disable process/network/file monitoring
  - **PowerShell** â€” disable script block logging
  - **Kernel-Process** â€” disable process creation events
  - **AMSI** â€” disable script content inspection (also achievable via `autopatch`)
- Pair with `auditpol -action stealth` and `autopatch` for comprehensive telemetry evasion
- Use `enable` for cleanup after operations â€” restores blinded providers to full verbose logging
...
- **T1082** â€” System Information Discovery (sessions/providers enumeration)
- **T1562.002** â€” Impair Defenses: Disable Windows Event Logging (stop action)
- **T1562.006** â€” Impair Defenses: Indicator Blocking (blind action)
