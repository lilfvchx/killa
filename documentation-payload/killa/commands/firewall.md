...
------------------------------------------------------------------------------------------------------------------------
KillaTestRule_12345                          In    Allow    TCP    true     9999
...
### Windows
- **COM API**: Uses `HNetCfg.FwPolicy2` and `HNetCfg.FWRule` COM objects â€” no subprocess spawning, no netsh.exe
...
- The `-name` parameter is not used on macOS â€” use `-program` with the application path instead
...
- **enable/disable**: Not supported â€” Linux has no global firewall toggle. Returns informative guidance.
