...
| netonly | No | false | Network-only credentials (Windows only â€” like `runas /netonly`) |

## Usage

### Windows â€” Run as different user

```
runas -command "cmd.exe /c whoami" -username CORP\admin -password AdminPass1
```

### Windows â€” Network-only mode
...
4. Returns the spawned process PID (fire-and-forget â€” output is not captured)
...
- **Linux/macOS as root:** Uses kernel-level `setuid`/`setgid` â€” most reliable and stealthy method.
...
- **Domain handling on Unix:** Domain prefixes (`DOMAIN\user` or `user@domain`) are stripped â€” only the local username is used for user lookup.
