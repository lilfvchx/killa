...
- **Windows**: Uses `WTSEnumerateSessionsW` + `WTSQuerySessionInformationW` â€” no subprocess creation.
- **Linux**: Natively parses `/var/run/utmp` binary (384-byte utmp records) â€” no subprocess creation. Shows active USER_PROCESS entries with PID, terminal, login time, and remote host.
