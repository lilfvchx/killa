...
- Assembly.Load() + entry point invocation â€” zero disk artifacts
...
3. Import resolution with IAT-level ExitProcess â†’ ExitThread hook (prevents agent death)
...
No file is ever written to disk â€” the binary exists only in an anonymous memory-backed file descriptor.
...
3. Ad-hoc code signing is applied (`codesign -s -`) â€” required on Apple Silicon (arm64)
...
- **Windows:** Auto-detects .NET (CLR header) vs native PE. Native EXEs use in-memory mapping with ExitProcess hooking, PEB command line patching, and TLS callback support â€” no temp file, no disk IOCs. Falls back to temp file if in-memory loading fails (e.g., complex dependencies).
...
- **macOS:** Binary must be a valid Mach-O executable (all 6 magic variants validated). Ad-hoc codesign is applied automatically â€” required on Apple Silicon.
- Static binaries work best â€” dynamically linked binaries require shared libraries on the target
