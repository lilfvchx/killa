...
func init() {
	agentstructs.AllPayloadData.Get("killa").AddCommand(agentstructs.Command{
		...
		Description:         "Perform process injection using memory sections (NtCreateSection/NtMapViewOfSection) to avoid RW->RX transitions and NtWriteVirtualMemory.",
		...
		TaskFunctionCreateTasking: func(taskData *agentstructs.PTTaskMessageAllData) agentstructs.PTTaskCreateTaskingMessageResponse {
			...
			createArtifact(taskData.Task.ID, "Process Inject", fmt.Sprintf("Section injection into PID %d (%d bytes)", int(pid), len(fileContents)))
