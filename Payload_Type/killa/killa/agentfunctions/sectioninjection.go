...
func init() {
	agentstructs.AllPayloadData.Get("killa").AddCommand(agentstructs.Command{
		...
		Description:         "Perform process injection using memory sections (NtCreateSection, NtMapViewOfSection)",
		HelpString:          "section-injection",
		Version:             1,
		MitreAttackMappings: []string{"T1055.001", "T1055.002"}, // Process Injection
		...
		TaskFunctionCreateTasking: func(taskData *agentstructs.PTTaskMessageAllData) agentstructs.PTTaskCreateTaskingMessageResponse {
			...
			// Resolve file contents by checking actual args
			...
			createArtifact(taskData.Task.ID, "Process Inject", fmt.Sprintf("NtCreateSection/NtMapViewOfSection into PID %d (%d bytes)", int(pid), len(fileContents)))
