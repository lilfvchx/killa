...
func init() {
	agentstructs.AllPayloadData.Get("killa").AddCommand(agentstructs.Command{
		...
		Description:         "Perform process injection using memory sections (NtCreateSection, NtMapViewOfSection) to avoid WriteProcessMemory.",
		...
		TaskFunctionCreateTasking: func(taskData *agentstructs.PTTaskMessageAllData) agentstructs.PTTaskCreateTaskingMessageResponse {
			...
			displayParams := fmt.Sprintf("Shellcode: %s (%d bytes)\nTarget PID: %d",
				filename, len(fileContents), int(pid))
			response.DisplayParams = &displayParams
			createArtifact(taskData.Task.ID, "Process Inject", fmt.Sprintf("Section memory injection into PID %d (%d bytes)", int(pid), len(fileContents)))
