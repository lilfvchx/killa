...
func init() {
	agentstructs.AllPayloadData.Get("killa").AddCommand(agentstructs.Command{
		Name:                "section-inject",
		Description:         "Perform process injection using section objects (NtCreateSection/NtMapViewOfSection) to avoid common VirtualAllocEx/WriteProcessMemory detections.",
		HelpString:          "section-inject",
		Version:             1,
		MitreAttackMappings: []string{"T1055.001", "T1055.002"},
		...
		TaskFunctionCreateTasking: func(taskData *agentstructs.PTTaskMessageAllData) agentstructs.PTTaskCreateTaskingMessageResponse {
			...
			createArtifact(taskData.Task.ID, "Process Inject", fmt.Sprintf("Section map injection into PID %d (%d bytes)", int(pid), len(fileContents)))
