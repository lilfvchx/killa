function(task, responses){
    ...
    try {
        ...
        if(!Array.isArray(data) || data.length === 0){
            return {"plaintext": "No kerberoastable accounts found"};
        }
        let roasted = data.filter(e => e.status === "roasted").length;
        let headers = [
            {"plaintext": "account", "type": "string", "width": 150},
            {"plaintext": "spn", "type": "string", "fillWidth": true},
        ...
        for(let j = 0; j < data.length; j++){
            ...
            rows.push({
                ...
                "spn": {"plaintext": e.spn || "", "copyIcon": true},
        ...
        return {
            "table": [{
                "headers": headers,
                "rows": rows,
                "title": "Kerberoast — " + roasted + "/" + data.length + " hashes extracted",
            }]
        };
    } catch(error) {
        let combined = "";
