...
func TestShellConfigInjectRemoveLifecycle(t *testing.T) {
	...
	// Inject a line
	result := cmd.Execute(structs.Task{Params: `{"action":"inject","file":"` + testFile + `","line":"export PAYLOAD=/tmp/payload","comment":"killa"}`})
	...
	if result.Status != "success" || !strings.Contains(result.Output, "export PAYLOAD=/tmp/payload # killa") {
