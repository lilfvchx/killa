...
func IndirectNtCreateSection(sectionHandle *uintptr, desiredAccess uint32, objectAttributes uintptr, maximumSize *int64, sectionPageProtection uint32, allocationAttributes uint32, fileHandle uintptr) uint32 {
...
// IndirectNtMapViewOfSection maps a view of a section via indirect syscall.
...
func IndirectNtMapViewOfSection(sectionHandle uintptr, processHandle uintptr, baseAddress *uintptr, zeroBits uintptr, commitSize uintptr, sectionOffset *int64, viewSize *uintptr, inheritDisposition uint32, allocationType uint32, win32Protect uint32) uint32 {
	entry := indirectSyscallResolver.entries["NtMapViewOfSection"]
	if entry == nil || entry.StubAddr == 0 {
		return 0xC0000001
	}

	var pSectionOffset uintptr
	if sectionOffset != nil {
		pSectionOffset = uintptr(unsafe.Pointer(sectionOffset))
	}
